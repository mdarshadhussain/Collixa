...
const LearningPathRoadmap: React.FC<LearningPathRoadmapProps> = ({ steps, isFallback = false }) => {
  if (!steps || steps.length === 0) return null

  return (
    <div className="space-y-8 relative">
      {isFallback && (
        <div className="mb-8 p-4 bg-amber-500/10 border border-amber-500/30 rounded-2xl flex items-start gap-3">
          <div className="p-2 bg-amber-500 rounded-full text-black shrink-0">
             <Clock size={14} />
          </div>
          <div>
            <p className="text-[10px] font-black uppercase tracking-widest text-amber-500 mb-1">AI Limit Reached</p>
            <p className="text-[11px] text-amber-200/70 italic leading-relaxed">
              Google Gemini is currently busy. Showing a high-quality standard roadmap for your goal. Try refreshing in 1 minute for a personalized AI path!
            </p>
          </div>
        </div>
      )}
