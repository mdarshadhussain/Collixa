  ...
  return (
    <>
      <div className="space-y-6 md:space-y-10">
        ...
        {/* Header Area */}
        <div className="flex flex-col gap-6 bg-[var(--color-bg-secondary)] border border-[var(--color-border)] rounded-2xl md:rounded-[3.5rem] p-5 sm:p-8 md:p-12 shadow-2xl shadow-[var(--color-accent)]/5 group overflow-hidden relative mt-0">
          ...
          <div className="space-y-3 max-w-none relative z-10">
            ...
            <p className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] text-[var(--color-text-secondary)] opacity-50 max-w-[280px] sm:max-w-none">Broadcast your vision. Find your perfect match.</p>
          </div>
          
          <div className="flex flex-col gap-4 relative z-10 w-full pt-4">
            <div className="relative w-full group">
              ...
              <input
                ...
                className="w-full pl-14 md:pl-16 pr-8 py-4 md:py-5 bg-[var(--color-bg-primary)] border border-[var(--color-border)] rounded-2xl text-[10px] md:text-[11px] font-bold uppercase tracking-widest focus:ring-1 focus:ring-[var(--color-accent)] transition-all shadow-inner"
              />
            </div>
            ...
            <div className="flex items-stretch gap-2 md:gap-4 w-full">
              <button
                ...
                className="aspect-square h-[52px] md:h-[64px] flex items-center justify-center rounded-2xl border border-[var(--color-border)] bg-[var(--color-bg-primary)] hover:bg-[var(--color-accent-soft)] transition-all text-[var(--color-accent)] shadow-sm group/refresh shrink-0"
              >
              ...
              <button 
                ...
                className="flex-1 h-[52px] md:h-[64px] px-4 md:px-10 border-2 border-[var(--color-accent)] text-[var(--color-accent)] text-[9px] md:text-[11px] font-black uppercase tracking-[0.2em] rounded-2xl hover:bg-[var(--color-accent)] hover:text-white transition-all shadow-lg shadow-[var(--color-accent)]/5 whitespace-nowrap flex items-center justify-center"
              >
                My Intents
              </button>
              <button 
                onClick={() => router.push('/create')}
                className="flex-1 h-[52px] md:h-[64px] px-4 md:px-10 bg-[var(--color-accent)] text-[var(--color-inverse-text)] text-[9px] md:text-[11px] font-black uppercase tracking-[0.2em] flex items-center justify-center gap-2 md:gap-3 transition-all hover:bg-[var(--color-inverse-bg)] shadow-xl shadow-[var(--color-accent)]/20 rounded-2xl group/btn whitespace-nowrap"
              >
        ...
        {/* Filter Pill List */}
        <div className="flex items-center gap-2 md:gap-3 overflow-x-auto pb-4 no-scrollbar -mx-4 px-4 sm:mx-0 sm:px-0">
          {CATEGORIES.map((cat) => (
            <button
              key={cat}
               onClick={() => setFilterCategory(cat)}
              className={`px-6 md:px-8 py-2.5 md:py-3.5 rounded-full text-[9px] md:text-[10px] font-bold uppercase tracking-[0.2em] transition-all whitespace-nowrap border ${
                filterCategory === cat 
               ...
               return (
                 ...
                 >
                   ...
                   <div className="p-4 md:p-6">
                      <span className="text-[8px] md:text-[9px] font-black uppercase tracking-[0.3em] text-[var(--color-accent)] mb-1 md:mb-2 block">{intent.category || 'General'}</span>
                      <h3 className="text-sm md:text-xl font-serif font-black text-[var(--color-text-primary)] leading-tight line-clamp-2 group-hover:text-[var(--color-accent)] transition-colors mb-3 md:mb-4 h-[2.2rem] md:h-[3rem]">
                        {intent.title}
                      </h3>
                      ...
                      <div className="flex items-center justify-between text-[var(--color-text-secondary)]">
                          <div className="flex items-center gap-1 md:gap-1.5">
                             ...
                             <span className="text-[7px] md:text-[8px] uppercase font-bold tracking-wider truncate max-w-[40px] md:max-w-[80px]">{intent.location || 'Remote'}</span>
                          </div>
                         <Badge 
                           variant={intent.status === 'completed' ? 'accent' : (intent as any).is_full ? 'outline' : 'sage'} 
                           className={`text-[7px] md:text-[9px] font-black px-2 py-0.5 ${intent.status === 'completed' ? '' : (intent as any).is_full ? 'border-red-500/50 text-red-500' : 'bg-[var(--color-accent-soft)]/20 text-[var(--color-accent)]'}`}
                         >
                           {intent.status === 'completed' ? 'Done' : (intent as any).is_full ? 'Full' : intent.status === 'in_progress' ? 'Live' : 'Open'}
                         </Badge>
                      </div>
                   </div>
                 </div>
