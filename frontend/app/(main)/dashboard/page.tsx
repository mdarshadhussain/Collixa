...
export default function DashboardPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      fetch(`${API_URL}/api/intents/hub/sections`)
        .then(res => res.json())
        .then(hubData => setSections(hubData.data))
        .catch(err => console.error('Hub sections fetch failed:', err))

      // Fetch private data with individual error handling
      intentService.getUserIntents(authUser.id)
        .then(setMyIntents)
        .catch(err => console.error('My Intents fetch failed:', err))
      ...
      skillService.getMyExchanges()
        .then(exchangesRes => {
          if (exchangesRes && exchangesRes.success && authUser) {
      ...
      intentService.getPendingActions()
        .then(pendingActions => {
          if (pendingActions) setPendingCollaborationRequests(pendingActions)
        })
        .catch(err => console.error('Pending actions fetch failed:', err))
  ...
  const totalPendingActionItems = pendingConversations.length + pendingExchanges.length + pendingCollaborationRequests.length
          ...
          <section className="md:col-span-1 bg-[var(--color-bg-secondary)] border border-[var(--color-border)] rounded-[2.5rem] p-8 flex flex-col">
            ...
            <div className="flex-1 space-y-4 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
              ...
              {pendingCollaborationRequests.map(req => (
                <div key={req.id} className="flex items-center gap-4 bg-[var(--color-bg-primary)] p-4 rounded-2xl border border-[var(--color-border)] hover:border-[var(--color-accent)]/50 transition-colors group">
                  <Avatar src={req.user?.avatar_url} name={req.user?.name} size="sm" />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold truncate">{req.user?.name}</p>
                    <p className="text-[10px] font-bold text-[var(--color-accent)]">Collaboration Request</p>
                    <p className="text-[8px] text-[var(--color-text-secondary)] truncate">{req.intent?.title}</p>
                  </div>
                  <button onClick={() => router.push(`/intent/${req.intent?.id}`)} className="bg-[var(--color-inverse-bg)] text-[var(--color-inverse-text)] p-3 rounded-xl group-hover:bg-[var(--color-accent)] transition-colors"><ArrowUpRight size={16} /></button>
                </div>
              ))}
              {totalPendingActionItems === 0 && (
